...
const classifyToken = (usd: number, sym: string) => {
  if (['USDC', 'USDT', 'DAI', 'WETH', 'ETH', 'WBTC'].includes(sym) || usd >= 1000) return 'core';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const chain of chainsToScan) {
      ...
      try {
        if (chainConfig.kind === 'solana') {
          ...
          const nativeBalance = lamports / Math.pow(10, chainConfig.decimals);
          ...
          if (nativeBalanceUsd > 0.01) {
            processedTokens.push({
              ...
              classification: classifyToken(nativeBalanceUsd, chainConfig.nativeSymbol),
          ...
          tokenAccounts.forEach(t => {
            ...
            processedTokens.push({
              ...
              classification: classifyToken(balanceUsd, symbol),
              price_usd: price,
              chain,
              has_unlimited_approval: false,
              logo_url: null,
          ...
          const nativeBalance = Number(sats) / Math.pow(10, chainConfig.decimals);
          const nativePrice = await fetchNativePriceUsd(chain);
          const nativeBalanceUsd = nativeBalance * nativePrice;
          ...
          const finalTokens = withMetadata.map(r => {
            ...
            return {
              ...
              balance_usd: balanceUsd,
              classification: classifyToken(balanceUsd, r.data!.symbol),
              price_usd: price,
              chain,
              has_unlimited_approval: false,
    ...
    const { error: tokensError } = await supabase.from('tokens').insert(tokensToInsert);
